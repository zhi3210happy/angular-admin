...
export class PersonListComponent implements OnInit {
    ...
    ngOnInit() {
        this.initTable();
       this.personManageService.ref.subscribe(()=>{
            this.initTable();
        })
    }
    ...
    initPerson(){
        this.personManageService.init().subscribe({
            next: () => { SweetAlert.alert('初始化数据库成功'); this.initTable(); },
            error: error => SweetAlert.warning('初始化数据库失败'),
        }
        )
    }
    // 添加人员
    addPerson() {
         if (this.tabControlService.isExist('/person-add')) {
    ...
    checkPerson() {
        if (this.selected.length !== 1) {
            ...
            const personName = this.selected[0].name;
